    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status',function($query){
                $html = '<select name="status" class="form-control review_status" data-id="'.$query->id.'">
                            <option '.($query->status === 0 ? 'selected' : '').'  value="pending" >Pending</option>
                            <option '.($query->status === 1 ? 'selected' : '').' value="approved">Approved</option>
                        </select>';
                return $html;
            })

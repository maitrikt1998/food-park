...
class ReservationDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function($query){
            $delete = "<a href='".route('admin.reservation.delete',$query->id)."' class='btn btn-danger delete-item ml-2'><i class='fas fa-trash'></i></a>";

            return $delete;
        })
        ->addColumn('status',function($query){
            $html = '<select name="status" class="form-control reservation_status" data-id="'.$query->id.'">
